...
int **read_board_from_file(char *filename)
{
    ...
    int **test_board = (int**)malloc(ROW_SIZE * sizeof(int*));
...
int is_board_valid() {
    ...
    for(int i =ROW_SIZE+COL_SIZE;i< NUM_OF_THREADS;i++ ) {
        params[i].id = i;
        for(int i =0;i<)
        int bounds = i - (ROW_SIZE+COL_SIZE);
        params[i].starting_row = bounds;
        params[i].starting_col = bounds;
        params[i].ending_row = bounds+2;
        params[i].ending_col = bounds+2;
    ...
    for(int i =0;i<NUM_OF_THREADS;i++) {
        printf("thread id:%d isvalid %d",worker_validation[params[i].id],worker_validation[i]);
    }
